...
Created a multi-container application that consists of a simple Python Flask web application and a Redis database. The Flask application used Redis to store and retrieve data.
Requirements
...
Bonus
    ...
    Persistent Storage for Redis: Configured Redis to use a volume to persist its data.
    Environment Variables: Modify the Flask application to read Redis connection details from environment variables and updated the docker-compose.yml accordingly.
